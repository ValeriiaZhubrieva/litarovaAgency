...
window.initSubmenu = function() {
  ...
  megaMenus.forEach((megaMenu) => {
    const submenuLinksWrapper = megaMenu.querySelector("[data-submenu-links]");
    const submenuItemsWrapper = megaMenu.querySelector("[data-submenu-items]");
    function closeAllSubmenus() {
      submenuItemsWrapper.querySelectorAll("[data-content]." + submenuActiveClass).forEach((submenu) => {
        submenu.classList.remove(submenuActiveClass);
      });
      submenuLinksWrapper.querySelectorAll("[data-category]." + submenuBtnActiveClass).forEach((btn) => {
      ...
      submenuItemsWrapper.style.setProperty("--submenu-height", `0px`);
      if (removeTrap) {
        removeTrap();
        removeTrap = null;
      }
    ...
    function openSubmenu(submenuName, { withFocusReturn = true } = {}) {
      closeAllSubmenus();
      const submenu = submenuItemsWrapper.querySelector(`[data-content="${submenuName}"]`);
      const button = submenuLinksWrapper.querySelector(`[data-category="${submenuName}"]`);
      if (submenu && button) {
    ...
    document.addEventListener("click", (e) => {
      ...
      const isInsideSubmenu = e.target.closest("[data-content]." + submenuActiveClass);
      if (btn) {
        if (btn.hasAttribute("data-submenu-click") || isTouch && isMobile.any()) {
          e.preventDefault();
        }
        const submenuName = btn.dataset.category;
    ...
    if (!isTouch && !isMobile.any()) {
      submenuLinksWrapper.querySelectorAll("[data-category]").forEach((button) => {
        if (button.hasAttribute("data-submenu-click")) return;
        const submenuName = button.dataset.category;
        const submenu = submenuItemsWrapper.querySelector(`[data-content="${submenuName}"]`);
        if (!submenu) return;
        let overBtn = false;
        ...
        const startCloseTimer = () => {
          ...
          closeTimer = setTimeout(() => {
            if (!overBtn && !overSubmenu) {
              ...
              submenuItemsWrapper.classList.remove(submenuActiveClass);
              submenuLinksWrapper.classList.remove(submenuBtnActiveClass);
              if (!document.querySelector("[data-content]." + submenuActiveClass)) {
                html.classList.remove("submenu-open");
              }
            }
          }, 300);
    ...
    function activateFirstSubmenu() {
      if (window.innerWidth >= 767.98) {
        const firstButton = submenuLinksWrapper.querySelector("[data-category]");
        if (firstButton) {
          const firstSubmenuName = firstButton.dataset.category;
          openSubmenu(firstSubmenuName, { withFocusReturn: false });
        }
      }
    }
    const menuObserver = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.attributeName === "class") {
          const isMenuOpen = megaMenu.classList.contains("is-open");
          if (isMenuOpen) {
    ...
    menuObserver.observe(megaMenu, { attributes: true });
  });
};
document.addEventListener("DOMContentLoaded", window.initSubmenu);
const langActiveParentBtns = document.querySelectorAll("[data-toggle-lang-active]");
if (langActiveParentBtns.length) {
  document.addEventListener("click", (e) => {
    langActiveParentBtns.forEach((btn) => {
      const langBlock = btn.closest(".lang-block");
      if (btn.contains(e.target)) {
        btn.classList.toggle("is-active");
        langBlock.classList.toggle("is-active");
      } else if (!langBlock.contains(e.target)) {
...
const swiperInstances = /* @__PURE__ */ new Map();
function destroySwiperForElement(el) {
  const inst = swiperInstances.get(el);
  if (!inst) return;
  if (typeof inst.destroy === "function") {
    inst.destroy(true, true);
  } else {
    console.warn("Swiper instance for element has no destroy() — dumping from map", el, inst);
  }
...
window.initTabsSliders = function() {
  document.querySelectorAll(".reviews__slider").forEach((el) => {
    ...
    const instance = new Swiper(el, {
      ...
      breakpoints: {
        319: { slidesPerView: 1.5, spaceBetween: 8 },
        549.98: { slidesPerView: 2.5, spaceBetween: 8 },
        991.98: { slidesPerView: 3.5, spaceBetween: 8 }
      }
    });
    swiperInstances.set(el, instance);
  });
  document.querySelectorAll(".reviews__slider-reverse").forEach((el) => {
    if (swiperInstances.has(el)) return;
    const instance = new Swiper(el, {
      modules: [Autoplay],
      observer: true,
...
if (document.querySelector("[data-fls-slider]")) {
  ...
  window.addEventListener("load", initTabsSliders);
}
    ...
    tabItems.forEach(function(item, index) {
      item.addEventListener("click", function() {
        ...
        if (tabContents[index]) {
          ...
          setTimeout(function() {
            tabContents[index].style.transition = "opacity 0.4s";
            tabContents[index].style.opacity = "1";
          }, 10);
        }
        const swipersInside = container.querySelectorAll(".swiper");
        swipersInside.forEach((el) => destroySwiperForElement(el));
        setTimeout(() => {
          window.initTabsSliders();
          swipersInside.forEach((el) => {
            const inst = swiperInstances.get(el);
